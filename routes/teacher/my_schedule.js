...
// Endpoint hapus jadwal
router.delete('/my_schedule/:id', (req, res) => {
    if (!req.session.userId || req.session.folderRole !== 'teacher') {
        return res.status(403).json({ message: 'Unauthorized' });
    }
    ...
    // Cek apakah jadwal sudah diisi siswa
    const checkSql = `SELECT student_id FROM schedules WHERE schedule_id = ? AND teacher_id = ?`;
    db.query(checkSql, [scheduleId, teacherId], (err, results) => {
        if (err) return res.status(500).json({ message: 'Database error', error: err });
        if (results.length === 0) return res.status(404).json({ message: 'Schedule not found' });

        const { student_id } = results[0];
        if (student_id !== null) {
            return res.status(400).json({ message: 'Schedule has a student. Please remove student first.' });
        }

        const deleteSql = `DELETE FROM schedules WHERE schedule_id = ? AND teacher_id = ?`;
        db.query(deleteSql, [scheduleId, teacherId], (err, results) => {
            if (err) return res.status(500).json({ message: 'Database error', error: err });
